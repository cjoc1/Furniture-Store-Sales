...
/*
1. Find the total sales revenue for each product: This query helps identify the top-selling products and understand which products are generating the most revenue. This information can be used to inform product development, marketing campaigns, and inventory management decisions.
*/
...
/*
Average order value (AOV) for each payment type: This query helps understand how different payment methods affect customer spending behavior. Businesses can use this information to optimize their payment processing options and potentially increase AOV.
*/
SELECT payment_type, AVG(price) AS average_order_value
FROM sales
GROUP BY payment_type;


/*
Number of customers who have made purchases in each state: This query helps identify the states with the most active customer bases. Businesses can focus their marketing efforts on these states to reach a larger customer base and increase sales.
*/
SELECT state, COUNT(DISTINCT name) AS number_of_customers
FROM sales
GROUP BY state;

/*
Top 10 most popular products in terms of sales volume: This query helps identify the products that are most in demand. Businesses can use this information to prioritize product development, allocate inventory, and plan marketing campaigns.
*/
SELECT product, COUNT(*) AS sales_volume
FROM sales
GROUP BY product
...
/*
Customer churn rate: This query helps identify customers who are at risk of churning, or stopping doing business with the company. Businesses can reach out to these customers with retention campaigns, personalized offers, or customer support initiatives to address any underlying issues and prevent them from discontinuing their patronage.
*/
SELECT
    name,
...
GROUP BY
    name
HAVING
    last_purchase_date < DATE('now', '-30 days');


/*
Correlation between customer latitude and average order value: This query helps identify geographical trends in customer spending behavior. Businesses can use this information to target specific regions with tailored marketing campaigns or pricing strategies.
*/
-- Identify geographical trends in customer spending behavior
SELECT
  city,
  state,
  country,
...
The first query calculates the total sales revenue for each product. The results show that the Bed product has generated the most revenue, followed by the Chair and Couch products.

Average Order Value for Each Payment Type:

The second query calculates the average order value for each payment type. The results show that Amex customers have the highest average order value, followed by Visa and Mastercard customers. Diners Club customers have the lowest average order value.

Number of Customers Who Have Made Purchases in Each State:

The third query counts the number of unique customers who have made purchases in each state. The results show that most customers reside in California, followed by Florida and Georgia.

Top 10 Most Popular Products in Terms of Sales Volume:
...
The sixth query identifies customers who have not made a purchase in the past 30 days. The results show that there are 15 customers who meet this criteria.

Correlation Between Customer Latitude and Average Order Value:

The seventh query identifies geographical trends in customer spending behavior by sorting cities, states, and countries based on their average order value. The results show that customers in New York City, NY have the highest average order value, followed by customers in Miami, FL and Burpengary, Queensland, Australia. Customers in Helsingin, Finland have the lowest average order value.
